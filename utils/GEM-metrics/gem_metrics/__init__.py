...
def compute(outs: Predictions, refs: Optional[References] = None, srcs: Optional[Sources] = None, metrics_dict: Dict[str, List] = None, pred_file: str = None) -> Dict:
    ...
    if refs is not None:
        ...
        for metric_class in metrics_dict['referenced_metrics']:
            ...
            metric = metric_class()
            ret = metric.compute(outs, refs)

            # if len(ret) > 1:
            #     scores_per_example = ret[1]

            #     with open(pred_file + ".eval.csv", "w") as f:
            #         for scores in zip(
            #                 scores_per_example["semsim"], 
            #                 scores_per_example["contr"], 
            #                 scores_per_example["agree"], 
            #                 scores_per_example["perpl"]
            #             ):
...
def process_files(config):
    ...
    # single-file mode
        ...
        values = compute(outs, refs, srcs, metric_dict, pred_file=config.predictions_file)
